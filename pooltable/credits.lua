-- pooltable/credits.lua
-- Credit/betting system for pool table (reuses chip denominations from casino games)
...
function M.increaseBet(bank)
  ...
  if bank.balance >= bank.currentBet + chipValue then
...
function M.placeBet(bank)
  if bank.currentBet <= 0 or bank.balance < bank.currentBet then
    return false
  end
  bank.balance = bank.balance - bank.currentBet
  return true
end

function M.canPlaceBet(bank)
  return bank.currentBet > 0 and bank.balance >= bank.currentBet
end
